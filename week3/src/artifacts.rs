...
impl Recorder {
    ...
    pub fn create(out_dir: &Path, l: usize, every: u64) -> Result<Self, String> {
...
pub fn write_run_json(config: &RunConfig, t_grid: &[f64]) -> Result<(), String> {
    ...
    let (update, time_unit) = match config.update {
        Update::Metropolis => ("metropolis", "sweep"),
        Update::Wolff => ("wolff", "cluster_flip"),
    };
