...
- [ ] Write the contract tests with `assert_cmd`: header exactly `T\tmean|M|\tacceptance` then one tab-separated row per temperature; `run.json` fields and values; `series.jsonl` keys, ramp order, `sweep` restart at 1, `M`/`E` integers when scaled by `1e6`, `M` equal to the signed mean of `spins` sums when both are recorded; `spins.jsonl` keys, `L^2` entries of `+1/-1`, only created when `every > 0`, and global `sweep` values `discard + k` at the first temperature and `discard + measure + discard + k` at the second; grid inclusion for `1.5..3.5` step `0.05` (41 values ending at 3.5) and step `0.1` (21 values, no 3.5), plus a unit test of `temperature_grid` for the `t_to` rule.
